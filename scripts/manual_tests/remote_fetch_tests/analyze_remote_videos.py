...
def test_single_video(video_path: str, expected_actions: list, video_name: str):
    ...
    config.yaw_threshold = 8.0
    config.pitch_threshold = 8.0
...
def main():
    video_dir = Path("output/remote_fetch/videos")
    action_file = Path("output/remote_fetch/动作对应.txt")
    ...
    print("=" * 80)
    print("分析 remote_fetch 视频的动作识别率")
    print("=" * 80)
    print()
    ...
    for action, stats in sorted(action_stats.items()):
        ...
        print(
            f"  {action:12s}  通过率：{pass_rate:5.1f}%  ({stats['passed']}/{stats['total']})  平均置信度：{avg_conf:.2%} (范围：{min_conf:.2%}-{max_conf:.2%})"
        )
    ...
    for result in valid_results:
        for detail in result["action_details"]:
            if detail.confidence < 0.50:
                print(
                    f"  ❌ {result['video']}: {detail.action} = {detail.confidence:.2%}"
                )
