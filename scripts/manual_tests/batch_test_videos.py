...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="批量测试视频动作检测",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
注意：
  - 所有视频的前 2 秒为基准段（面向屏幕不动）
  - 动作对应.txt 中的动作列表不包含基准段
  - 例如：6 秒视频 = 2 秒基准 + 4 秒动作时间
        """,
    )
    parser.add_argument("--video-dir", default="15", help="视频目录（默认：15）")
    parser.add_argument(
        "--action-file",
        default=None,
        help="动作对应文件（默认：<video-dir>/动作对应.txt）",
    )
    parser.add_argument(
        "--output",
        default="output/results/batch_test_result.txt",
        help="结果输出文件（默认：output/results/batch_test_result.txt）",
    )
    args = parser.parse_args()

    video_dir = Path(args.video_dir)
    action_file = (
        Path(args.action_file) if args.action_file else video_dir / "动作对应.txt"
    )
    ...
    print("=" * 80)
    print()
    print("⚠️  注意：所有视频的前 2 秒为基准段（面向屏幕不动）")
    print("    动作时间 = 视频总时长 - 2 秒基准段")
    print()
